...
// Xact3DSound.cpp
//
// The sample demonstrates the use of X3DAudio engine with XACT using the xact3d3.h
// helper header. For direct use of X3DAudio with XACT, see the XactCustom3DSound sample
//
// XACT allows 3D values to be used to control a variety of sound parameters -- and all
// from within the content creator's purview. This sample demonstrates just a few;
// programmatically, the sounds are just 3D-positioned, but runtime parameter controls
// (RPCs) authored in the XACT authoring tool can be used to tell sounds exactly how to
// get quieter, more filtered, or how to adjust their wet/dry reverberation mix with
// distanace or relative orientation.
//
// For more information, see the XACT project file (XACT3DSounds.xap), which includes
// comments in the 'notes' fields for each RPC explaining what behaviors they are
// performing. Or double click them in the XACT authoring tool to see their specific
// curves.
//
...
X3DAUDIO_DISTANCE_CURVE g_VolumeCurve;
X3DAUDIO_DISTANCE_CURVE_POINT g_VolumeCurvePoints[] =
{
    {0.0f, 1.0f},
    {1.0f, 0.0f},
};
...
class Sample : public ATG::Application
{
    ...
    XACTVARIABLEINDEX m_indexVolume;             // Variable Index
...
HRESULT Sample::Initialize()
{
    ...
    // Initialize the 3D settings
    hr = XACT3DInitialize( m_pXACTEngine, m_X3DInstance );
    if( FAILED( hr ) )
        ATG::FatalError( "XACT3DInitialize failed with error %#X\n", hr );
    ...
    // Set up volume curve; only be used by XACT cues not assigned to use a runtime
    // parameter control (RPC) that maps the "Distance" variable.
    g_VolumeCurve.PointCount = 2;
    g_VolumeCurve.pPoints = g_VolumeCurvePoints;
    ...
    m_Emitter.pVolumeCurve = &g_VolumeCurve;
    m_Emitter.pLFECurve = &g_VolumeCurve;
    ...
    if( FAILED( hr = ATG::LoadFilePhysicalMemory( "game:\\media\\sounds\\Xact3DSounds.xwb",
                                                  &pbWaveBank,
                                                  &dwFileSize ) ) )
        ATG::FatalError( "Could not load file \"Xact3DSounds.xwb\", failed with error %#X\n", hr );
    ...
    if( FAILED( hr = ATG::LoadFile( "game:\\media\\sounds\\Xact3DSounds.xsb",
                                    &pbSoundBank,
                                    &dwFileSize ) ) )
        ATG::FatalError( "Could not load file \"Xact3DSounds.xsb\", failed with error %#X\n", hr );
...
HRESULT Sample::Update()
{
    ...
    HRESULT hr = XACT3DCalculate( m_X3DInstance, &m_Listener, &m_Emitter, &m_DSPSettings );
    if( FAILED( hr ) )
        ATG::FatalError( "XACT3DCalculate failed with error %#X\n", hr );

    if( m_pCue )
    {

        hr = XACT3DApply( &m_DSPSettings, m_pCue );
        if( FAILED( hr ) )
            ATG::FatalError( "XACT3DApply failed with error %#X\n", hr );

        DWORD dwCueState;

        m_pCue->GetState( &dwCueState );
        ...
        m_pCue->SetVariable( m_indexVolume, ( XACTVARIABLEVALUE )fDecibel );

        //
        // you must currently call XACT3DApply once before playing a cue instance
        //
        if( XACT_CUESTATE_PREPARED == dwCueState )
        {

            if( FAILED( hr = m_pCue->Play() ) )
            {

                ATG::FatalError( "Cue play failed with %#X\n", hr );
...
HRESULT Sample::PrimeSound( DWORD dwIndex )
{
    ...
    if( m_bPlaying )
    {
        ...
        m_indexVolume = m_pCue->GetVariableIndex( "Volume" );
...
HRESULT Sample::Render()
{
    ...
    else
    {
        ...
        m_Font.DrawText( 0, 0, 0xffffffff, L"Xact3DSound" );
		...
		m_Font.DrawText(  16, fYPos+=40, 0xff808080,  L"XACT variables:" );

		swprintf_s( strBuffer, L"%.3f", m_DSPSettings.DopplerFactor );
        m_Font.DrawText(  16, fYPos+=20, 0xffffffff, L"DopplerPitchScalar" );
        m_Font.DrawText( 256, fYPos,     0xffffff00, strBuffer );

        swprintf_s( strBuffer, L"%.3f", m_DSPSettings.EmitterToListenerDistance );
        m_Font.DrawText(  16, fYPos+=20, 0xffffffff, L"Distance" );
        m_Font.DrawText( 256, fYPos,     0xffffff00, strBuffer );

        swprintf_s( strBuffer, L"%.3f", 360.0f * m_DSPSettings.EmitterToListenerAngle / X3DAUDIO_2PI );
        m_Font.DrawText(  16, fYPos+=20, 0xffffffff, L"OrientationAngle (degrees)" );
        m_Font.DrawText( 256, fYPos,     0xffffff00, strBuffer );
