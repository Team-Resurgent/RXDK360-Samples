...
//--------------------------------------------------------------------------------------
bool CZipFile::LoadZipFile( const CHAR* filename )
{
    ...
    m_data = new CHAR[fileSizeLow];
    DWORD bytesRead = 0;
    if( !ReadFile( file, m_data, fileSizeLow, &bytesRead, 0 ) )
    {
        CloseHandle( file );
        Free();
    ...
    m_zipFilename = _strdup( filename );
...
bool CZipFile::MakeDirectory()
{
    ...
    EndOfCentralDir* pEndOfCentralDir = ( EndOfCentralDir* )
        ( m_data + m_dataSize - sizeof( EndOfCentralDir ) );
    ...
    for( DWORD i = 0; i < m_numEntries; ++i )
    {
        ...
        CentralDirFileHeader* pFileHeader = ( CentralDirFileHeader* )( m_data + currentOffset );
...
//--------------------------------------------------------------------------------------
void CZipFile::GetFilename( DWORD index, CHAR* fileBuffer, DWORD bufSize ) const
{
    assert( index >= 0 && index < m_numEntries );
    const CentralDirFileHeader* pFileHeader =
        ( const CentralDirFileHeader* )( m_data + m_centralDirOffsets[index] );
    ...
    strncpy_s( fileBuffer, bufSize, filename, _TRUNCATE );
}
...
//       The index must be in range.
//--------------------------------------------------------------------------------------
void CZipFile::GetFile( DWORD index, CHAR** pData, DWORD* pSize ) const
{
    assert( index >= 0 && index < m_numEntries );
    const CentralDirFileHeader* pFileHeader =
        ( const CentralDirFileHeader* )( m_data + m_centralDirOffsets[index] );
...
//--------------------------------------------------------------------------------------
bool CZipFile::GetFile( const CHAR* filename, CHAR** pData, DWORD* pSize ) const
{
...
void CZipFile::Free()
{
    ...
    free( const_cast<CHAR*>( m_zipFilename ) );
    m_zipFilename = 0;
