...
public:
    ...
    bool    GetFile( const CHAR* filename, CHAR** pData, DWORD* pSize ) const;

    // Get the filename for the specified file. Index must be less than GetCount().
    void    GetFilename( DWORD index, CHAR* fileBuffer, DWORD bufSize ) const;

    // Get the file data for the specified file. Index must be less than GetCount().
    void    GetFile( DWORD index, CHAR** pData, DWORD* pSize ) const;
