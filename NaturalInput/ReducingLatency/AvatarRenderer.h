...
static const DWORD JOINT_BUFFER_COUNT = NUM_FRAME_BUFFERS;  // double buffered to allow skinning using vfetch
...
public:
    ...
    VOID Render( XMMATRIX matWorld, XMMATRIX matView, XMMATRIX matProj, const BOOL bShadowPass = FALSE );
    VOID Update()
    {
        RebuildJoints();
    }
    ...
    inline VOID SetSingleBuffering() { m_bSingleBuffer = TRUE; }
    inline VOID SetDoubleBuffering() { m_bSingleBuffer = FALSE; }
    inline VOID SetAvatarRenderedFence( const DWORD dwFence ) { m_dwAvatarRenderedFence = dwFence; }
...
private:
    ...
    HRESULT RenderOpaque(       DWORD dwModelIndex, D3DVertexBuffer* pJointBuffer, const BOOL bShadowPass );
    HRESULT RenderTransparent(  DWORD dwModelIndex, D3DVertexBuffer* pJointBuffer, const BOOL bShadowPass );
    HRESULT PostRender( );
...
private:
    ...
    DWORD                       m_dwAvatarRenderedFence;    // Fence on GPU of when avatar has finished rendering
    BOOL                        m_bSingleBuffer;            // Single buffer or double buffer resources
};
