...
const float4x4 c_model        : register( c8 );
const float4 highlights[16]   : register( c100 );
...
struct VSDepth_OUT
{
    ...
    float2 oPosZW         : TEXCOORD0;
};

[optimizeAutoZ(true)]
VSDepth_OUT WriteSkinnedDepthVS( in Vertex_c vertex )
{
    VSDepth_OUT output;
    
    float3 vDiscard1, vDiscard2, vDiscard3;
    
    ApplySkinning( vertex, vDiscard1, vDiscard2, output.oPosition, vDiscard3 );
    
    output.oPosZW = output.oPosition.zw;
