...
HRESULT AvatarRenderer::CreateAvatar( XAVATAR_METADATA& metadata )
{
    ...
    ATG::LoadVertexShader( "game:\\media\\shaders\\AvatarBody_vs.xvu", &bodyVs );
    ATG::LoadVertexShader( "game:\\media\\shaders\\AvatarHeadOpaque_vs.xvu", &headOpaqueVs );
    ATG::LoadVertexShader( "game:\\media\\shaders\\AvatarBodyShiny_vs.xvu", &bodyShinyVs );
    ...
    ATG::LoadPixelShader( "game:\\media\\shaders\\AvatarBodyOpaque_ps.xpu", &bodyOpaquePs );
    ATG::LoadPixelShader( "game:\\media\\shaders\\AvatarHeadOpaque_ps.xpu", &headOpaquePs );
    ATG::LoadPixelShader( "game:\\media\\shaders\\AvatarBodyTransparent_ps.xpu", &bodyTranspPs );
    ATG::LoadPixelShader( "game:\\media\\shaders\\AvatarBodyShinyOpaque_ps.xpu", &bodyShinyOpaquePs );
    ATG::LoadPixelShader( "game:\\media\\shaders\\AvatarBodyShinyTransparent_ps.xpu", &bodyShinyTranspPs );
...
HRESULT AvatarRenderer::RebuildJoints()
{
    ...
    for ( ; ; )
    {
        ...
        // If using single buffering for low latency and we rely on the
        // appliclation to make sure that the GPU won't be using the resources
        // while trying to update.
        if ( m_bSingleBuffer )
        {
            dwNextBufferIndex = dwCurrBufferIndex;

            // Make sure the GPU has finished with this resource if single buffered
            PIXBeginNamedEvent( 0, "Block Until GPU Done With Avatar" );
            m_pd3dDevice->BlockOnFence( m_dwAvatarRenderedFence );
            PIXEndNamedEvent();
        }
        ...
        #ifdef _DEBUG
        {
            ...
            if( pCurrJointBuffer->IsSet( m_pd3dDevice ) || pCurrJointBuffer->IsBusy( ) )
            {
...
//--------------------------------------------------------------------------------------
VOID AvatarRenderer::Render( XMMATRIX matWorld, XMMATRIX matView, XMMATRIX matProj, const BOOL bShadowPass )
{

    // Set up device state and shader constants
    PreRender( &matWorld, &matView, &matProj, bShadowPass );

    // Iterate over all component models and render opaque materials.
    for ( DWORD cmi = 0; cmi < m_pAssets->ComponentCount; ++cmi )
    {
        RenderOpaque( cmi, &m_JointBufferVBs[ m_dwJointBufferIndex ], bShadowPass );
    }

    // Iterate over all component models and render transparent materials.
    for ( DWORD cmi = 0; cmi < m_pAssets->ComponentCount; ++cmi )
    {
        RenderTransparent( cmi, &m_JointBufferVBs[ m_dwJointBufferIndex ], bShadowPass );
    }
                                    ...
                                    const BOOL          bShadowPass )
{
    ...
    m_pd3dDevice->SetVertexShaderConstantF( 8, (FLOAT*)modelTransform, 4 );
...
//--------------------------------------------------------------------------------------
HRESULT AvatarRenderer::RenderOpaque( DWORD dwModelIndex, D3DVertexBuffer* pJointBuffer, const BOOL bShadowPass )
{
...
//       function to set up the render state appropriately for the pass.
//--------------------------------------------------------------------------------------
HRESULT AvatarRenderer::RenderTransparent( DWORD dwModelIndex, D3DVertexBuffer* pJointBuffer, const BOOL bShadowPass )
{
    XAVATAR_MODEL*      pModel = &m_pAssets->pComponentModels[ dwModelIndex ];
    D3DVertexBuffer*    pVertexBuffer = &m_VertexBuffers[ dwModelIndex ];
    D3DIndexBuffer*     pIndexBuffer = &m_IndexBuffers[ dwModelIndex ];
