...
//--------------------------------------------------------------------------------------
VOID CameraManager::DisplayPIP( FLOAT yOffset)
{
    static const FLOAT drawWidth = 120.0f;
    static const FLOAT drawHeight = 90.0f;
    static const FLOAT drawX = 70.0f;
    static const FLOAT drawY = 600.0f;
    m_pip.BeginRender();
    m_pip.RenderDepthStream( drawX, drawY + yOffset, drawWidth, drawHeight );
    m_pip.RenderSkeletons(drawX, drawY + yOffset, drawWidth, drawHeight );
    m_pip.RenderColorStream( drawX + drawWidth + 5.0f, drawY + yOffset, drawWidth, drawHeight );
    m_pip.EndRender();
...
{    
    ...
    return ( SUCCEEDED( hrSkeleton ) );//SUCCEEDED ( hrImage ) && SUCCEEDED ( hrDepth ) && SUCCEEDED ( hrSkeleton ) );
}
...
VOID CameraManager::ApplyTiltCorrection( NUI_SKELETON_FRAME *pSkeleton )
{
    ...
    XMMATRIX matTranslateToOrigin = XMMatrixTranslation( -fAverageSpine.x, -fAverageSpine.y, -fAverageSpine.z );
    XMMATRIX matTranslateFromOrigin = XMMatrixTranslation( fAverageSpine.x, -fAverageSpine.y, fAverageSpine.z );
    XMMATRIX matTransformation = matLevel;
