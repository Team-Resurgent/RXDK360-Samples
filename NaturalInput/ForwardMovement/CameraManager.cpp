...
VOID CameraManager::DisplayPIP()
{
    ...
    m_pip.RenderSingleSkeleton(0, drawX, drawY, drawWidth, drawHeight );
...
//--------------------------------------------------------------------------------------
BOOL CameraManager::Update( DWORD dwFilterFlags )
{    
    ...
    // if the write is completed, then go to the next one
...
VOID CameraManager::ApplyTiltCorrection( NUI_SKELETON_FRAME *pSkeleton )
{
    ...
    XMVECTOR vDet;
    XMMATRIX matLevel = XMMatrixInverse(&vDet, NuiTransformMatrixLevel( vAverageNormalToGravity ) );
