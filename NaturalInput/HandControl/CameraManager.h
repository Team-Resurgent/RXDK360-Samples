...
public:
    ...
    VOID CopyCustomTexture( D3DTexture *pCustom );

    inline const IDirect3DTexture9* GetDepthVis80x60Texture ()
    {
        return m_p80x60DepthVis;
    };
    inline const IDirect3DTexture9* GetDepthVis320x240Texture ()
    {
        return m_pip.GetVis320x240Texture();
    };
    ...
    inline INT GetTrackedSkeletonIndex()
    {
        for ( INT iIndex = 0; iIndex < NUI_SKELETON_COUNT; ++iIndex )
        {
            if ( m_SkeletonFrame.SkeletonData[iIndex].eTrackingState == NUI_SKELETON_TRACKED ) 
