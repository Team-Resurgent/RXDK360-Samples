...
VOID CameraManager::DisplayPIP()
{
    ...
    static const FLOAT drawY = 70 ;

    m_pip.BeginRender();
    m_pip.RenderDepthStream( drawX, drawY, drawWidth, drawHeight );
    m_pip.RenderCustomDepthStream( drawX + drawWidth + 5, drawY, drawWidth, drawHeight, m_p80x60DepthVis );
...
VOID CameraManager::CopyCustomTexture(D3DTexture *pCustom )
{
    D3DLOCKED_RECT DepthRect;
    D3DLOCKED_RECT RGBRect;

    pCustom->LockRect(0, &DepthRect, NULL, 0 );
