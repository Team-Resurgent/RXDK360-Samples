...
HRESULT InitializeSkeletonTracking( D3DDevice* pd3dDevice )
{
    ...
    // Initialize ATG's debug visualization for NUI data
...
//--------------------------------------------------------------------------------------
// Name: FindClosestSkeletonToSensor()
// Desc: Returns the tracking id of the player closest to the sensor
//--------------------------------------------------------------------------------------

BOOL FindClosestSkeletonToSensor( DWORD* pTrackingID)
{
    ...
    if ( iClosestBody != -1 )
    {
        *pTrackingID = g_SkeletonFrame.SkeletonData[ iClosestBody ].dwTrackingID;
        PIXEndNamedEvent();
        return true;
    }
