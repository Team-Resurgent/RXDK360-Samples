...
//--------------------------------------------------------------------------------------
// Name: GetClosestPlayerIndex()
// Desc: Returns the skeleton index of the closest player to the sensor
//--------------------------------------------------------------------------------------
INT CameraManager::GetClosestPlayerIndex() const
{
    return ( m_iClosestSkeleton < 0 ||
             m_iClosestSkeleton >= NUI_SKELETON_COUNT ) ? 0 : m_iClosestSkeleton;
}

//--------------------------------------------------------------------------------------
// Name: SelectClosestSkeleton()
// Desc: Selects the closest skeleton to the camera to actively track
//--------------------------------------------------------------------------------------
VOID CameraManager::SelectClosestSkeleton()
{
    ...
    for ( UINT i = 0; i < NUI_SKELETON_COUNT; i++ )
    {
        ...
        XMVECTOR vTemp = XMVectorSubtract( m_SkeletonFrame.SkeletonData[ i ].Position, XMVectorZero() );
    ...
    if ( iClosestBody != -1 )
    {
        dwTrackingIDs[ 0 ] = m_SkeletonFrame.SkeletonData[ iClosestBody ].dwTrackingID;
    }
    ...
    m_iClosestSkeleton = iClosestBody;
}
