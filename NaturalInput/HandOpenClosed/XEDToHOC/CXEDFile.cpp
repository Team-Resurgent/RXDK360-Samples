...
UINT XEDMultiFrameRange::GetTotalFrameRange() const
{
    ...
    {
        bHasValidData = TRUE;
        startFrame = std::min( color.start.frameNumber, startFrame );
        endFrame = std::max( color.end.frameNumber, endFrame );
    }

    if ( depth.IsValid() )
    {
        bHasValidData = TRUE;
        startFrame = std::min( depth.start.frameNumber, startFrame );
        endFrame = std::max( depth.end.frameNumber, endFrame );
    }

    if ( skeleton.IsValid() )
    {
        bHasValidData = TRUE;
        startFrame = std::min( skeleton.start.frameNumber, startFrame );
        endFrame = std::max( skeleton.end.frameNumber, endFrame );
    }
...
UINT64 XEDMultiFrameRange::GetTotalTimeRange() const
{
    ...
    {
        bHasValidData = TRUE;
        startTime = std::min( color.start.timestampInUsec, startTime );
        endTime = std::max( color.end.timestampInUsec, endTime );
    }

    if ( depth.IsValid() )
    {
        bHasValidData = TRUE;
        startTime = std::min( depth.start.timestampInUsec, startTime );
        endTime = std::max( depth.end.timestampInUsec, endTime );
    }

    if ( skeleton.IsValid() )
    {
        bHasValidData = TRUE;
        startTime = std::min( skeleton.start.timestampInUsec, startTime );
        endTime = std::max( skeleton.end.timestampInUsec, endTime );
    }
...
UINT CXEDFile::GetEventIndexFromFrameNumber( XEDFRAMETYPE frameType, UINT frameNumber )
{
    ...
    case XED_TITLE_DATA:
        {
            // assume title data event is written after depth frame has been received
            const UINT uDepthEvent = XedGetNuiDepthEventIndexFromFrameNumber( m_pFileContext, frameNumber );
            UINT64  eventUSeconds;
            if( SUCCEEDED( XedReadNuiDepthFrame( m_pFileContext, uDepthEvent, NULL, &eventUSeconds, NULL, 320 * 240 * 4, FALSE ) ) )
            {
                return  XedGetTitleDataEventIndexFromMicroseconds( m_pFileContext, eventUSeconds + 100 );
            }
...
// Desc: Reads a Color frame from the specified event index in the file.
//----------------------------------------------------------------------------------------------------------------------
HRESULT CXEDFile::ReadFrame( UINT eventIndex, CXEDTitleDataFrame* pOutFrame )
{
    assert( IsOpen() && "File is not open" );
    assert( pOutFrame != NULL && "Frame pointer is null.");
    ...
    UINT uBufLength;
    HRESULT hr = XedReadTitleData( m_pFileContext, eventIndex, NULL, NULL, NULL, 0, &uBufLength );
    if( FAILED( hr ) )
        return hr;

    if( FAILED( hr = pOutFrame->AllocateBuffer( uBufLength ) ) )
        return hr;

    pOutFrame->m_frameInfo.frameNumber = 0;
    hr = XedReadTitleData( m_pFileContext, eventIndex, &pOutFrame->m_frameInfo.timestampInUsec, NULL, pOutFrame->GetData(), pOutFrame->GetBufferLength(), &uBufLength );

    assert( uBufLength <= pOutFrame->GetBufferLength() );

    if ( SUCCEEDED( hr ) )
    {
        pOutFrame->m_frameInfo.eventIndex = eventIndex;
        pOutFrame->m_frameInfo.type = XED_TITLE_DATA;
    }
...
//----------------------------------------------------------------------------------------------------------------------
// Name: CXEDTitleDataFrame::GetBufferLength
// Desc: Returns the size of the Color frame buffer in bytes.
//----------------------------------------------------------------------------------------------------------------------
DWORD CXEDTitleDataFrame::GetBufferLength() const
{
...
// Desc: Allocates enough memory to store a single frame of color buffer information.
//----------------------------------------------------------------------------------------------------------------------
HRESULT CXEDTitleDataFrame::AllocateBuffer( UINT uBufLength )
{
    if( uBufLength > m_uBufferSize )
    {
        if( m_pBuffer )
            free( m_pBuffer );
        m_pBuffer = (DWORD*)malloc( uBufLength );
        m_uBufferSize = uBufLength;
    }

    return (m_pBuffer == NULL) ? E_OUTOFMEMORY : S_OK;
}
...
// Desc: Destroy the frame, freeing any allocated buffer.
//----------------------------------------------------------------------------------------------------------------------
CXEDTitleDataFrame::~CXEDTitleDataFrame()
{
    if( m_pBuffer )
        free( m_pBuffer );
}