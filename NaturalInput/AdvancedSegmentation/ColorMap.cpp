...
VOID ColorMap::CopyToTexture( IDirect3DTexture9* pTexture ) const
{
...
//--------------------------------------------------------------------------------------
// Name: Copy()
// Copies contents of input color map to this map
//--------------------------------------------------------------------------------------
VOID ColorMap::Copy( const ColorMap& map )
{
...
//--------------------------------------------------------------------------------------
VOID ColorMap::Fill( const DWORD* pWords, const UINT stride )
{
...
//--------------------------------------------------------------------------------------
VOID ColorMap::Fill( const RGBAValue color )
{
    for( UINT y = 0; y < COLOR_MAP_HEIGHT; y++ )
    {
        for( UINT x = 0; x < COLOR_MAP_WIDTH; x++ )
...
//--------------------------------------------------------------------------------------
VOID ColorMap::SelectPixels( const BinaryMap& selectMap )
{
...
// The input select map is assumed to be half the size of the color map (as per NUI).
//--------------------------------------------------------------------------------------
VOID ColorMap::SelectPixels( const ColorMap& map, const BinaryMap& selectMap )
{
    const RGBAValue backgroundColor(0);
    for( UINT y = 0; y < COLOR_MAP_HEIGHT; y++ )
    {
...
//--------------------------------------------------------------------------------------
VOID ColorMap::SobelEdgeDetect( const ColorMap& inputMap )
{
...
// classification is done via BinaryMask where possible.
//--------------------------------------------------------------------------------------

VOID ColorMap::SelectForeground( const ColorMap& originalImage, const IntensityMap& edgeDifferences,
                                 const BinaryMap& fgPixels, const BinaryMap& edgePixels,
                                 const CumulativeMovingAverageBuffer& cma, BinaryMap& segmentationMap )
{
    const RGBAValue backgroundColor( 0 );
            ...
            else
            {
                if( edgePixels.Value( x >> 1, y >> 1 ) )
                {
                    ...
                    else
                    {
                        ...
                        {
                            // Now all the CMA can tell us is if the pixel is close to the background
                            // color. That is NOT the same as being foreground or background - it just
                            // means close in color. Think of a grey t-shirt against a grey wall - the
                            // CMA is likely to say 'yeah looks similar'.
                            // Fall back to comparing depth map and distances.
                ...
                {
                    // Pixel is neither silhouette nor foreground -> background
                    SetValue( x, y, backgroundColor );
                }
            }
        }
...
VOID ColorMap::SelectForegroundClassify( const ColorMap& originalImage, const IntensityMap& edgeDifferences,
                                 const BinaryMap& fgPixels, const BinaryMap& edgePixels,
                                 const CumulativeMovingAverageBuffer& cma, BinaryMap& segmentationMap )
{
    const RGBAValue backgroundColor( 0 );
    ...
    for( UINT y = 0; y < COLOR_MAP_HEIGHT; y++ )
    {
        for( UINT x = 0; x < COLOR_MAP_WIDTH; x++ )
        {
            if( fgPixels.Value( x >> 1, y >> 1 ) )
            {
                // Pixel is definitely part of the foreground
                const RGBAValue originalColor = originalImage.Value( x, y );
                SetValue( x, y, RGBAValue(0,0,127) );
            }
            else
            {
                if( edgePixels.Value( x >> 1, y >> 1 ) )
                {
                    ...
                    {
                        // Pixel is definitely foreground, from edge difference map
                        SetValue( x, y, RGBAValue(0,255,0) );
                    }
                    else
                    {
                        // Use the cumulative moving average buffer to decide
                        ...
                        if( distance > FOREGROUND_DIFFERENCE_THRESHOLD_SQ)
                        {
                            SetValue( x, y, RGBAValue(255,0,0) );
                        }
                        else
                        {
                            // Now all the CMA can tell us is if the pixel is close to the background
