//--------------------------------------------------------------------------------------
// CumulativeMovingAverageBuffer.cpp
//
...
static __forceinline RGBAValue UpdateColor( const RGBAValue currentColor, const RGBAValue newColor )
{
    UINT red = currentColor.GetRed() + ( newColor.GetRed() - currentColor.GetRed() ) /
        CumulativeMovingAverageBuffer::UPDATE_FRACTION;
...
static __forceinline VOID Update2x2ColorPixels( const UINT x, const UINT y, ColorMap& currentColorMap,
                                                const ColorMap& newColorMap, BinaryMap& validMap )
{
...
//--------------------------------------------------------------------------------------
VOID CumulativeMovingAverageBuffer::Update( const ColorMap& newColorImage )
{
    ...
    for( UINT y = 0; y < BinaryMap::BINARY_MAP_HEIGHT; y++ )
    {
        for( UINT x = 0; x < BinaryMap::BINARY_MAP_WIDTH; x++ )
        {
...
//--------------------------------------------------------------------------------------
int gp;
VOID CumulativeMovingAverageBuffer::UpdateSelected( const ColorMap& newColorImage, const BinaryMap& selectMap )
{
    // Run through pixels in map. 
