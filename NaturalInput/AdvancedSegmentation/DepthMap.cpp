...
//--------------------------------------------------------------------------------------
VOID DepthMap::CopyToTexture( IDirect3DTexture9* pTexture ) const
{
...
// depth range.
//--------------------------------------------------------------------------------------
VOID DepthMap::CopyToTexture( IDirect3DTexture9* pTexture, const CatmullRomColorSpline& spline ) const
{
    assert( pTexture != NULL );
    assert( pTexture->GetLevelCount() == 1 );
...
//--------------------------------------------------------------------------------------
VOID DepthMap::Fill( const USHORT* pValues, const UINT stride )
{
...
//--------------------------------------------------------------------------------------
VOID DepthMap::Fill( const DepthValue value )
{
    for( UINT y = 0; y < DEPTH_MAP_HEIGHT; y++ )
    {
        for( UINT x = 0; x < DEPTH_MAP_WIDTH; x++ )
...
VOID DepthMap::Sobel( const DepthMap& inputMap )
{
