...
//--------------------------------------------------------------------------------------
// Name: KinectSensor::KinectSensor()
...
//--------------------------------------------------------------------------------------
HRESULT KinectSensor::Initialize(  DWORD dwFlags, DWORD dwHardwareThreadSkeleton )
{
    ...
    HRESULT hr = NuiInitialize( dwFlags, dwHardwareThreadSkeleton );
	...
	hr = NuiImageStreamOpen( NUI_IMAGE_TYPE_DEPTH_80x60, NUI_IMAGE_RESOLUTION_80x60, 0, 1, NULL, &m_hDepth80x60 );
    if ( FAILED (hr) )
    {
...
{
    if ( SUCCEEDED ( pKinectFrame->hrImageRetrieved ) && pKinectFrame->pImageFrame != NULL )
    {
        pKinectFrame->hrImageRetrieved = NuiImageStreamReleaseFrame( m_hImage, pKinectFrame->pImageFrame );
        pKinectFrame->pImageFrame = NULL;
    }

    if ( SUCCEEDED ( pKinectFrame->hr80x60Retrieved ) && pKinectFrame->pDepthFrame80x60 != NULL )
    {
        pKinectFrame->hr80x60Retrieved = NuiImageStreamReleaseFrame( m_hDepth80x60, pKinectFrame->pDepthFrame80x60 );
        pKinectFrame->pDepthFrame80x60 = NULL;
    }

    if ( SUCCEEDED ( pKinectFrame->hr320x240Retrieved ) && pKinectFrame->pDepthFrame320x240 != NULL )
    {
        pKinectFrame->hr320x240Retrieved = NuiImageStreamReleaseFrame( m_hDepth320x240, pKinectFrame->pDepthFrame320x240 );
        pKinectFrame->pDepthFrame320x240  = NULL;
    }
