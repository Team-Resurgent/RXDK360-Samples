...
// ForceFeedback.cpp
//
// This sample demonstrates a first person based navigation system using the torso for control.
//
...
HRESULT Sample::Update()
{
    ...
    if ( bNewFrameReceived )
    {
        ...
        for (UINT uCurrentSkeleton = 0; uCurrentSkeleton < NUI_SKELETON_COUNT; uCurrentSkeleton++)
        {
            ...
            {
                m_Person.UpdateNUI( &m_CameraManager.GetSkeleton()->SkeletonData[uCurrentSkeleton] );
                break;
            }
        }        
    }
...
HRESULT Sample::Render()
{
    ...
    // Render the atrium and the feet queue
    m_Environment.Draw( m_matView, m_matProj, m_Person.GetTurnAnglePlayerFacingDirection(), m_Person.GetForwardSpeed(), m_Person.GetStrafeSpeed() );
    m_CameraManager.DisplayPIP();
    // Render the HUD
    m_Font.Begin();
    m_Font.SetScaleFactors( 1.2f, 1.2f );
    m_Font.DrawText( 0.0f, 0.0f, 0xffffffff, L"First Person Natural" );
