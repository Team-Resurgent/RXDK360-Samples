...
HRESULT Sample::Initialize()
{
    ...
    // Load the Atrium's graphical resources
...
HRESULT Sample::Update()
{
    ...
    m_Person.Update( fElapsedTime );

    // Update the view from the person's position and facing direction.
    XMFLOAT3 vPositionF3 = m_Person.GetPosition();
    XMVECTOR vPosition = XMLoadFloat3( &vPositionF3 );
    XMFLOAT3 vFacingF3 = m_Person.GetFacing();
...
HRESULT Sample::Render()
{
    ...
    //Draw atrium
    m_Environment.Draw( m_matView, m_matProj, m_Person.GetFacing() );
    // Draw feet and compass wheel
    m_CameraManager.DisplayPIP();
    m_Environment.DrawUI( m_Person.GetFacing(), m_Person.GetForwardSpeed(), 
                        m_Person.GetStrafeSpeed(), m_Person.GetWheelColor() );

    // Render the HUD
    m_Font.Begin();
    m_Font.SetScaleFactors( 1.2f, 1.2f );
     m_Font.DrawText( 0.0f, 0.0f, 0xffffffff, L"First Person" );
    m_Font.SetScaleFactors( 1.0f, 1.0f );
    m_Font.DrawText( 0.0f, 0.0f, 0xffffff00, m_Timer.GetFrameRate(), ATGFONT_RIGHT );
    
    if ( m_Person.GetHandOnWheel() )
    {
        m_Font.DrawText( 800.0f, 150.0f, 0xFFFFFFFF, L"Move Right Hand Left and Right");
        m_Font.DrawText( 800.0f, 175.0f, 0xFFFFFFFF, L"Lower Hand to Detach");
    }
    else 
    {
        m_Font.DrawText( 800.0f, 150.0f, 0xFFFFFFFF, L"Lift Right Hand to Drive");
    }

    if ( m_bJointSmoothingEnabled )
    {
