...
inline UINT64 XEDMultiFrameRange::GetFirstFrameTime() const
{
    ...
    if ( color.IsValid() )
    {
        firstTime = min( color.start.timestampInUsec, firstTime );
    }
    
    if ( depth.IsValid() )
    {
        firstTime = min( depth.start.timestampInUsec, firstTime );
    }

    if ( skeleton.IsValid() )
    {
        firstTime = min( skeleton.start.timestampInUsec, firstTime );
    }
...
inline UINT64 XEDMultiFrameRange::GetLastFrameTime() const
{
    ...
    if ( color.IsValid() )
    {
        lastTime = max( color.start.timestampInUsec, lastTime );
    }

    if ( depth.IsValid() )
    {
        lastTime = max( depth.start.timestampInUsec, lastTime );
    }

    if ( skeleton.IsValid() )
    {
        lastTime = max( skeleton.start.timestampInUsec, lastTime );
    }
