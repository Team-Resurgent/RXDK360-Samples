...
public:
    ...
    static XMMATRIX g_matCameraViewProj;    // Global camera view*projection matrix
    static XMMATRIX g_matShadowViewProj;    // Global shadow view*projection matrix
    static XMVECTOR g_vLightDirection;      // Glogal light direction
    static XMVECTOR g_vViewPosition;        // Global viewer position

public:
    BOOL RenderCallback( DWORD dwSubset, const ATG::MESH_SUBSET* pSubset, DWORD dwFlags )
    {
        // Pass the  matrices to the vertex shader
        const DWORD VSCONST_matWorldViewProj = 0;  // World-View-Proj matrix
        ...
        XMMATRIX matCameraWVP = XMMatrixTranspose( m_matWorld * g_matCameraViewProj );
        XMMATRIX matShadowWVP = XMMatrixTranspose( m_matWorld * g_matShadowViewProj );
        ...
        ATG::g_pd3dDevice->SetVertexShaderConstantF( VSCONST_matWorldViewProj, ( float* )&matCameraWVP, 4 );
        ATG::g_pd3dDevice->SetVertexShaderConstantF( VSCONST_matShadow, ( float* )&matShadowWVP, 4 );
