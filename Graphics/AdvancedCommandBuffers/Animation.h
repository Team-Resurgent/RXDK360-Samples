...
PoseDecomp
{
    ...
    VOID StoreTransform( DWORD dwIndex, const XMMATRIX matTransform )
    {
    ...
    VOID BlendPoses( const PoseDecomp* __restrict pSrcPoseA, const PoseDecomp* __restrict pSrcPoseB, FLOAT fLerpFactor )
    {
        ...
        for( DWORD i = 0; i < m_dwPoseSize; ++i )
        {
            ...
            XMStoreFloat4A( &m_pPositionsOrientations[dwArrayIndex], vBlendedOrientation );
            dwArrayIndex += 2;
        }
    }
};
...
struct Pose3x4
{
    ...
    VOID        StoreTransform( const DWORD dwIndex, const XMMATRIX matTransform )
    {
    ...
    VOID        StoreTransformHalf( const DWORD dwIndex, const XMMATRIX matTransform )
    {
        // Writing the 3x4 matrix to memory.
        // Note the usage of write barriers here; they instruct the compiler not to
        // reorder the writes, in case we're writing to write-combined memory.
