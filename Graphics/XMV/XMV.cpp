...
HRESULT Sample::Update()
{
    ...
    else
    {
        ...
        {
            // Load the file into memory and then play it.
            // Other options include starting from a file handle, to allow playing
            // a movie that is packed into a file with other data, or playing a
            // movie using custom I/O routines.
            m_bFailed = FALSE;

            DWORD bufferSize;
            HRESULT hr = ATG::LoadFile( g_strMovieName, &m_movieBuffer, &bufferSize );
            if( SUCCEEDED( hr ) )
            {
                // Set the parameters to load the movie from memory.
                XmvParams.createType = XMEDIA_CREATE_FROM_MEMORY;
                XmvParams.createFromMemory.pvBuffer = m_movieBuffer;
                XmvParams.createFromMemory.dwBufferSize = bufferSize;

                hr = XMedia2CreateXmvPlayer( m_pd3dDevice, m_pXAudio2, &XmvParams, &m_xmvPlayer );
            }

            if( SUCCEEDED( hr ) )
            {
                // Reset to the default rotation and scaling.
                m_angle = 0.0;
                m_xScale = 1.0;
                m_yScale = 1.0;
                InitVideoScreen();
