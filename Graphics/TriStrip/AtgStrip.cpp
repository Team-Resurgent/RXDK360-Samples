...
int CStripper::CreateIndices( BOOL bStartCW, int iNumTriangles, int* pTriangles, WORD* pStripVerts )
{
    ...
    for( int i = 0; i < iNumTriangles; i++ )
    {
        ...
        if( i < iNumTriangles - 1 )
        {
            ...
            for( out_edge = 0; out_edge < 2; out_edge++ )
...
// Desc: Convert a tri-strip to a tri-list.
//--------------------------------------------------------------------------------------
DWORD ConvertDegensToResetIndex( DWORD dwNumInputIndices, const WORD* pInputIndices,
                                 WORD* pOutputIndices )
{
    DWORD dwNumOutputIndices = 0;

    DWORD start_ptr, end_ptr;

    start_ptr = end_ptr = 0;

    WORD ind0 = 0;
    WORD ind1 = pInputIndices[0];
    WORD ind2 = pInputIndices[1];

    for( DWORD src = 2; src < dwNumInputIndices; src++ )
    {
        ind0 = ind1;
        ind1 = ind2;
        ind2 = pInputIndices[src];

        // Check for degenerates (note we ignore degens where ind0 == ind2 because
        // replacing them with a reset index would make the strip worse)
        if( ind0 != ind1 && ind1 != ind2 )
        {
            // Update where the current strip ends.
            end_ptr = src;
        }
        else
        {
            // End of a strip
            if( start_ptr + 2 <= end_ptr )
            {
                // Good strip
                if( start_ptr & 1 )
                {
                    if( ( end_ptr - start_ptr + 1 ) & 1 )
                    {
                        // Odd winding and odd length => output the strip reversed 
                        for( int i = int( end_ptr ); i >= int( start_ptr ); i-- )
                            pOutputIndices[dwNumOutputIndices++] = pInputIndices[i];
                    }
                    else
                    {
                        // Odd winding and even length => add a degen to fix winding
                        pOutputIndices[dwNumOutputIndices++] = pInputIndices[start_ptr];

                        for( int i = int( start_ptr ); i <= int( end_ptr ); i++ )
                            pOutputIndices[dwNumOutputIndices++] = pInputIndices[i];
                    }
                }
                else
                {
                    // Even winding => output the strip
                    for( int i = int( start_ptr ); i <= int( end_ptr ); i++ )
                        pOutputIndices[dwNumOutputIndices++] = pInputIndices[i];
                }

                // Append the reset index
                pOutputIndices[dwNumOutputIndices++] = 0xffff;
            }

            // Set start_ptr assuming the next time src is incremented the
            // strip will be non-degenerate.
            start_ptr = src - 1;
        }
    }

    // Output the last strip.
    if( start_ptr + 2 <= end_ptr )
    {
        // Good strip
        if( start_ptr & 1 )
        ...
        {
            // Output the strip, even winding
            for( int i = int( start_ptr ); i <= int( end_ptr ); i++ )
                pOutputIndices[dwNumOutputIndices++] = pInputIndices[i];
        }
    }

    return dwNumOutputIndices;
}
                               ...
                               DWORD dwFlags )
{
    ...
    *ppStripIndices = 0;
    ...
    else if( dwFlags & ATGTRISTRIPPER_USE_RESET_INDEX )
    {
        // Use a reset index instead of degens to stitch strips together.
        WORD* pTempIndices = new WORD[dwBestIndexCount * 2];

        dwBestIndexCount = ConvertDegensToResetIndex( dwBestIndexCount, *ppStripIndices,
                                                      pTempIndices );
                                            ...
                                            DWORD dwNumVertices, WORD** ppVertexPermutation )
{
    ...
    {
        WORD index = pStripIndices[i];

        assert( index == 0xffff || ( index < dwNumVertices && index >= 0 ) );
        if( index != 0xffff && pSortTable[index].iFirstUsed == -1 )
