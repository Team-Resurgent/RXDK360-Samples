...
//
// Common defines, enums, methods, and macros for the tiled runtime.
//
...
#include <xnamath.h>
#include <assert.h>
#include <stdio.h>    // _vsnprintf_s (DebugSpewV); pulled in transitively by MSVC's
#include <stdarg.h>   // <hash_map> before, but include it directly where it is used
...
namespace TiledRuntime
{
    ...
    // parameters, e.g. _vsnprintf_s below). The const carried no meaning; drop it.
    static inline VOID DebugSpewV( const CHAR* strFormat, va_list pArgList )
    {
        CHAR str[2048];
...
namespace TiledRuntime
{
    ...
    // Desc: For the given page data format, return the number of bytes per texel.
    //       Compressed formats return 0 and are handled separately.
    //--------------------------------------------------------------------------------------
    inline UINT GetBytesPerTexel( const PageDataFormat DataFormat )
    {
        switch( DataFormat )
        {
        case PDF_8BPP:
    ...
    // Desc: Returns a single page's dimensions in texels, for the given format.  Note that
    //       compressed formats return their texel dimensions, not their block dimensions.
    //--------------------------------------------------------------------------------------
    inline SIZE GetPageSizeTexels( const PageDataFormat DataFormat )
    {
        SIZE TexelSize = { 0, 0 };
    ...
    // Desc: Returns the width of a page border in the physical page atlas, in texels.
    //       Uncompressed formats have 1 texel borders, while compressed formats have 4 texel
    //       borders to accommodate a single compression block as the border.
    //--------------------------------------------------------------------------------------
    inline UINT GetPageBorderTexelCount( const PageDataFormat DataFormat )
    {
        switch( DataFormat )
        {
        case PDF_BC1_4:
