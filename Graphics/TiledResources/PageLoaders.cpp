...
{
    ASSERT( pTileID->TileID != D3DTILED_INVALID_PHYSICAL_ADDRESS );
    ASSERT( pTileID->VTileID != D3DTILED_INVALID_VIRTUAL_ADDRESS );
    ...
    pTileID->pResource->GetLevelDesc( pTileID->MipLevel, &SurfDesc );
    ...
    m_pTilePool->UpdateTileContents( pTileID->TileID, pBuffer, SurfDesc.Format );

    m_pTilePool->MapVirtualTileToPhysicalTile( pTileID->VTileID, pTileID->TileID );
...
VOID* MandelbrotTileLoader::CreateThreadContext()
{
    ...
    // uncompressed buffer is sized for the largest tile size possible
...
HRESULT MandelbrotTileLoader::LoadAndMapTile( TrackedTileID* pTileID, VOID* pThreadContext )
{
    ...
    D3DTiledTexture* pResource = pTileID->pResource;
    D3DTILED_SURFACE_DESC MipDesc;
    pResource->GetLevelDesc( pTileID->MipLevel, &MipDesc );
    ...
    if( pResource->GetType() == D3DSRTYPE_ARRAYTEXTURE )
    {
        ...
        BOOL IsQuilted = pSATexture->GetQuiltSize( &QuiltWidth, &QuiltHeight );
        if( IsQuilted )
        {
    ...
    case D3DFMT_CTX1:
        {
            UINT WidthBlocks = MipDesc.TileTexelWidth / 4;
            POINT DestPoint = { 0, 0 };
            RECT SrcRect = { 0, 0, MipDesc.TileTexelWidth, MipDesc.TileTexelHeight };

            const UINT BlockSizeBytes = 8;

            assert( pLC->pUncompressedBuffer != NULL );

            XGCompressSurface( pLC->pBuffer, 
                WidthBlocks * BlockSizeBytes, 
                MipDesc.TileTexelWidth, 
                MipDesc.TileTexelHeight, 
                (D3DFORMAT)MAKELINFMT( MipDesc.Format ), 
                &DestPoint, 
                pLC->pUncompressedBuffer, 
                MipDesc.TileTexelWidth * sizeof(UINT),
                (D3DFORMAT)MAKELINFMT( D3DFMT_A8R8G8B8 ),
                &SrcRect, 
                0,
                0.5f );
    ...
    m_pTilePool->UpdateTileContents( pTileID->TileID, pBuffer, MipDesc.Format );
...
//--------------------------------------------------------------------------------------
inline FLOAT ImagMagnitude( XMVECTOR Value )
{
...
{
    UINT Count = 0;
    while( ImagMagnitude( Point ) <= 4.0f && Count < 512 )
    {
...
//--------------------------------------------------------------------------------------
inline D3DCOLOR CreateColor( const BYTE Pattern, const BYTE Tile, const BYTE MinAlpha, const BOOL DebugColoring )
{
...
//--------------------------------------------------------------------------------------
VOID MandelbrotTileLoader::CreateMandelbrot( BYTE* pBuffer, UINT TileX, UINT TileY, UINT ArraySlice, const D3DTILED_SURFACE_DESC& MipLevelDesc, const XMVECTOR vQuiltScale, const XMVECTOR vQuiltOffset )
{
    ...
    // texels.
    BYTE MinAlpha = 255;
    if( MipLevelDesc.Format == D3DFMT_DXT3A || MipLevelDesc.Format == D3DFMT_DXT5A )
    {
        MinAlpha = 0;
    }
        ...
        for( UINT X = 0; X < MipLevelDesc.TileTexelWidth; ++X )
        {
            ...
            case D3DFMT_R5G6B5:
                *(WORD*)pPixel = TiledRuntimeTest::Color32To565( ColorARGB );
                ...
                break;
            case D3DFMT_A4R4G4B4:
                *(WORD*)pPixel = TiledRuntimeTest::Color32To4444( ColorARGB );
                if( m_Grid && ( X == 0 || Y == 0 ) )
                {
                    *(WORD*)pPixel = 0;
                }
            ...
            case D3DFMT_L8:
                *(BYTE*)pPixel = TiledRuntimeTest::Color32To8( ColorARGB );
                ...
                break;
            case D3DFMT_A16B16G16R16:
                *(UINT64*)pPixel = TiledRuntimeTest::Color32To64ABGR( ColorARGB );
                if( m_Grid && ( X == 0 || Y == 0 ) )
                {
                    *(UINT64*)pPixel = TiledRuntimeTest::Color32To64ABGR( D3DCOLOR_ARGB( 255, 0, 0, 0 ) );
                }
                break;
            case D3DFMT_A32B32G32R32F:
                {
            ...
            case D3DFMT_A2R10G10B10:
                *(DWORD*)pPixel = TiledRuntimeTest::Color32To210ARGB( ColorARGB );
                if( m_Grid && ( X == 0 || Y == 0 ) )
                {
                    *(DWORD*)pPixel = TiledRuntimeTest::Color32To210ARGB( D3DCOLOR_ARGB( 255, 0, 0, 0 ) );
                }
                break;
            case D3DFMT_A8R8G8B8:
            default:
                *(DWORD*)pPixel = ColorARGB;
                if( m_Grid && ( X == 0 || Y == 0 ) )
                {
                    *(DWORD*)pPixel = D3DCOLOR_ARGB( 255, 0, 0, 0 );
                }
                break;
            }
        }
...
TiledFileLoader::TiledFileLoader()
{
    ...
    m_pDefaultTile = NULL;
    m_DefaultPhysicalTile = D3DTILED_INVALID_PHYSICAL_ADDRESS;
...
//       resource.
//--------------------------------------------------------------------------------------
HRESULT TiledFileLoader::CreateTiledTexture2D( D3DTiledResourceDevice* pTiledResourceDevice, D3DTiledTexture** ppTexture )
{
    ...
    static const D3DFORMAT TiledFormatToD3DFormat[] =
    {
        D3DFMT_L8,              // TILED_FORMAT_8BPP
        D3DFMT_R5G6B5,          // TILED_FORMAT_16BPP_B5G6R5
        D3DFMT_A1R5G5B5,        // TILED_FORMAT_16BPP_B5G5R5A1
        D3DFMT_A4R4G4B4,        // TILED_FORMAT_16BPP_B4G4R4A4
        D3DFMT_A8R8G8B8,        // TILED_FORMAT_32BPP_R8G8B8A8
        D3DFMT_A2R10G10B10,     // TILED_FORMAT_32BPP_R10G10B10A2
        D3DFMT_A16B16G16R16,    // TILED_FORMAT_64BPP_R16G16B16A16
        D3DFMT_A16B16G16R16F,   // TILED_FORMAT_64BPP_R16G16B16A16F
        D3DFMT_DXT1,            // TILED_FORMAT_BC1
        D3DFMT_DXT3,            // TILED_FORMAT_BC2
        D3DFMT_DXT5,            // TILED_FORMAT_BC3
        D3DFMT_DXN,             // TILED_FORMAT_BC4
        D3DFMT_DXT5A,           // TILED_FORMAT_BC5
        (D3DFORMAT)0,           // TILED_FORMAT_BC6
        (D3DFORMAT)0,           // TILED_FORMAT_BC7
        D3DFMT_L16,             // TILED_FORMAT_16BPP_R16
        D3DFMT_G8R8,            // TILED_FORMAT_16BPP_R8G8
    };
    C_ASSERT( ARRAYSIZE(TiledFormatToD3DFormat) == TiledContent::TILED_FORMAT_MAX );

    D3DFORMAT Format = TiledFormatToD3DFormat[m_Header.Format];
    if( Format == (D3DFORMAT)0 )
    {
        return E_FAIL;
    }

    return pTiledResourceDevice->CreateTexture( NULL, m_Header.BaseWidthTexels, m_Header.BaseHeightTexels, m_Header.MipLevelCount, Format, ppTexture );
}
...
{
    if( m_DefaultPhysicalTile == D3DTILED_INVALID_PHYSICAL_ADDRESS || m_Header.DefaultPage == TiledContent::TILEDFILE_INVALID_LOCATOR )
    {
        return TRUE;
    }
    ...
    BOOL IsDefaultTile = ( Locator == m_Header.DefaultPage );
...
HRESULT TiledFileLoader::LoadAndMapTile( TrackedTileID* pTileID, VOID* pThreadContext )
{
    ...
    {
        if( m_DefaultPhysicalTile == D3DTILED_INVALID_PHYSICAL_ADDRESS )
        {
            ASSERT( pTileID->TileID != D3DTILED_INVALID_PHYSICAL_ADDRESS );

            // fill tile
            m_pTilePool->UpdateTileContents( pTileID->TileID, m_pDefaultTile, MipDesc.Format );
            ...
            m_DefaultPhysicalTile = pTileID->TileID;

            DebugSpew( "Pinned Tile ID: %I64u\n", m_DefaultPhysicalTile );
        }
        else
        {
            ASSERT( pTileID->TileID == D3DTILED_INVALID_PHYSICAL_ADDRESS );
...
{
    ASSERT( pTileID->VTileID != D3DTILED_INVALID_VIRTUAL_ADDRESS );
    ASSERT( pTileID->TileID != m_DefaultPhysicalTile );
...
//--------------------------------------------------------------------------------------
HRESULT TiledFileLoader::LoadFile( const CHAR* strFileName )
{
    // Open the file.
    ...
    if( m_Header.Magic != TiledContent::TILEDFILE_HEADER_MAGIC )
    {
        CloseHandle( hFile );
        return E_INVALIDARG;
    }
