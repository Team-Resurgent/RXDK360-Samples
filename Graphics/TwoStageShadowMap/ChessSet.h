...
public:
    ...
    {
        if( m_bMoving )
            m_matObject = XMMatrixRotationY( XM_PI * fElapsedTime ) * m_matObject;
    }
...
public:
    ...
    INT                GetMovingPieceCount()
    {
        BOOL iCount = 0;

        for( DWORD i = 0; i < 32; i++ )
        {
            if( m_pChessPieces[i] && m_pChessPieces[i]->m_bMoving )
            {
                ++iCount;
            }
        }
    ...
    VOID                ChangeMovingPieceCount( BOOL mIncrement )
    {
        for( DWORD i = 0; i < 32; i++ )
        {
            if( m_pChessPieces[i] && ( m_pChessPieces[i]->m_bMoving != mIncrement ) )
            {
                m_pChessPieces[i]->m_bMoving = mIncrement;
                return;
            }
        }
    }
    ...
    VOID                Update( FLOAT fElapsedTime )
    {
        ...
        {
            if( m_pChessPieces[i] )
                m_pChessPieces[i]->Update( fElapsedTime );
        }
    }
    ...
    VOID        RenderPieces( XMMATRIX matWorld, BOOL bRenderStationary = TRUE, BOOL bRenderMoving = TRUE )
    {
        for( DWORD i = 0; i < 32; i++ )
        {
            if( m_pChessPieces[i] )
            {
                BOOL bMoving = m_pChessPieces[i]->m_bMoving;
                BOOL bRender = ( bRenderStationary && !bMoving ) || ( bRenderMoving && bMoving );
                if( bRender )
                {
