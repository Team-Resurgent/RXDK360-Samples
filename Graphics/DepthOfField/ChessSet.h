...
// External variables
//--------------------------------------------------------------------------------------
extern XMMATRIX g_matView;    // Global access to the view matrix
extern XMMATRIX g_matProj;    // Global access to the projection matrix
...
public:
    ...
    {
        // Pass the world, view, and projection matrices to the vertex shader
        ...
        XMVECTOR fDet;
        XMMATRIX matWorldInv = XMMatrixInverse( &fDet, m_matWorld );
        XMMATRIX matWorldView = XMMatrixMultiply( m_matWorld, g_matView );
        XMMATRIX matWorldViewProj = XMMatrixMultiply( matWorldView, g_matProj );
        XMMATRIX matWorldViewInv = XMMatrixInverse( &fDet, matWorldView );
        XMMATRIX matWorldViewT = XMMatrixTranspose( matWorldView );
        XMMATRIX matWorldViewProjT = XMMatrixTranspose( matWorldViewProj );

        ATG::g_pd3dDevice->SetVertexShaderConstantF( VSCONST_vCameraPt, ( FLOAT* )&matWorldViewInv._41, 1 );
        ATG::g_pd3dDevice->SetVertexShaderConstantF( VSCONST_matWorldView, ( FLOAT* )&matWorldViewT, 4 );
        ATG::g_pd3dDevice->SetVertexShaderConstantF( VSCONST_matWorldViewProj, ( FLOAT* )&matWorldViewProjT, 4 );

        // Put light and camera in object space
        static XMVECTOR vLightDir = XMVectorSet( 1.0f, 1.0f, 1.0f, 1.0f );
        XMVECTOR vLightDirOS = XMVector3TransformNormal( XMVector3Normalize( vLightDir ), matWorldInv );
        ATG::g_pd3dDevice->SetPixelShaderConstantF( PSCONST_vLightDir, ( FLOAT* )&vLightDirOS, 1 );
